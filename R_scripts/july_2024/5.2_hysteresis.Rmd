...
# Define arrow positions manually for each panel
...
forward_arrows <- arrow_positions %>%
  ...
  group_by(Type) %>%
  summarise(nearest_point = list(find_nearest_point(cur_data(), Fishing_Arrow)), .groups = "drop") %>%
  unnest_wider(nearest_point)

reverse_arrows <- arrow_positions %>%
...
# Save figure as a PDF file
pdf("../../output/figures/Fig_5B_arrows.pdf", width=2.5, height=8.5)
