...
# Create the plot
pdf("../../output/figures/fig_S13_v3.pdf", width = 9, height = 12)
...
# Create the plot
pdf("../../output/figures/fig_S14_v1.pdf", width = 9, height = 12)
...
# Create the plot
pdf("../../output/figures/fig_S15_v1.pdf", width = 9, height = 12)
...
# Create the plot
pdf("../../output/figures/fig_S16_v2.pdf", width = 9, height = 12)
...
# Define the initial conditions for which to evaluate
y_values <- seq(0, 0.9, by=0.1)
T0_values <- seq(0, 0.85, by=0.1)
f_values<-seq(0.1, 0.7, by=0.1)
...
# Create the plot
pdf("../../output/figures/fig_S17.pdf", width = 9, height = 12)
...
# Reorder factor levels
beta_all$Id <- factor(beta_all$Id, levels = c("Herbivores\neven", "Browser\ndominated", "Grazer\ndominated", "Generalist\ndominated"))
...
# Create the plot
pdf("../../output/figures/fig_S18.pdf", width = 9, height = 12)

ggplot(data = beta_all, aes(betaT, betaM)) +
  geom_tile(aes(fill = Final_Coral)) +
  scale_x_continuous("betaT - Expansion of Turf",
                     # expand = c(0,0),                     # remove padding between axis line and data
                     breaks = c(0, 0.3, 0.6, 0.9),
                     labels = c(0,0.3,0.6,0.9)) +
                     # breaks = c(0,0.2,0.4,0.6,0.8),
                     # labels = c(0,0.2,0.4,0.6,0.8)) +          
  scale_y_continuous("betaM - Expansion of Macroalgae",
                     # expand = c(0,0),                     # remove padding between axis line and data
                     breaks = c(0, 0.3, 0.6, 0.9),
                     labels = c(0,0.3,0.6,0.9)) +      
  labs(fill = "Final\nCoral\nCover") +
